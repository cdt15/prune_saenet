...
def estimate_adj_matrix_for_stacked_imputed_datasets(X_list, causal_order, ad_weight_type="min", weights=None, prior_knowledge=None, seed=None):
    """ _estiamte_adjacency_matrix for pruning assuming a common pattern
    ...
    causal_order : list, length n_features
        Causal order.
    ad_weight_type : string, "min" or "1se"
        Name of methods to decide adaptive weights.
    weights : array-like of shape (n_samples, ), optional (default=None)
        Proportion observed for each row in the un-imputed data. 
